...
def save_model(args, epoch, ckpt_path, model, model_without_ddp, optimizer, loss_scaler):
    ...
    if loss_scaler is not None:
        ...
        save_on_master(to_save, ckpt_path)
    else:
        client_state = {'epoch': epoch}
        model.save_checkpoint(save_dir=args.output_dir, tag="checkpoint-%s" % epoch_name, client_state=client_state)


def load_model(args, ckpt_path, model_without_ddp, optimizer, loss_scaler):
...
def get_params_groups2(model, head_name):
    regularized = []
    not_regularized = []
    transhead = []
    for name, param in model.named_parameters():
        ...
        if name.endswith(".bias") or len(param.shape) == 1:
            ...
            regularized.append(param)
    return [{'params': regularized}, {'params': not_regularized, 'weight_decay': 0.}, {'params': transhead}]
